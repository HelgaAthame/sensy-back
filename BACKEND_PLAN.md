...
**Фаза 5 — довески. 🚧 Частично выполнено.**
- ✅ Чат/уведомления: `GET/POST api/chat/:chatType/message`, системные триггеры (звонок обработан, высокий негатив, ошибка анализа, GPT-саммари готово/упало) — подключены в `AnalysisProcessor`/`GptAnalysisService`, проверены локально и в браузере.
- ⬜ Excel-экспорт (`GET api/download/mediafile/excel`) — не реализован вообще.
- ⬜ Полировка ошибок/статусов — без конкретики, требует отдельного прохода по API при появлении реальных жалоб от фронта.
- ⬜ Нагрузочное тестирование на реальном железе — не проводилось.

**Известные ограничения/незавершённое (не блокирует основной функционал, но стоит держать в уме):**
- **Анализ тональности отключён в проде** (`ENABLE_TONAL_ANALYSIS=false` на Render) — 512MB RAM бесплатного тарифа не хватает даже одному Whisper-tiny с учётом базового потребления NestJS/Prisma, не говоря про вторую ONNX-модель. STT и поиск по словарям работают в полном объёме, эмоции — нет. Решается либо платным планом Render/другим хостингом, либо более лёгкой SER-моделью, если такая найдётся.
- **`seen` у ChatMessage не синхронизируется с фронтом** — поле есть в схеме/DTO, но нет `PUT`-эндпоинта пометки "прочитано"; фронт сейчас считает непрочитанные чисто на клиенте через localStorage. Можно добавить позже, если понадобится честная синхронизация между устройствами.
- **Нет автоматических тестов** — Jest настроен (`package.json`), но ни одного `*.spec.ts` не написано. Для растущего API стоит начать хотя бы с smoke-тестов на критичные эндпоинты.
- **Нет CI-гейта** — пуш в `dev` сразу деплоится на прод через Render автодеплой, без промежуточного стейджинга или обязательного прогона тестов/линта перед деплоем.
- **Профиль пользователя не реализован** — только `POST auth/signin`, нет смены пароля/эндпоинтов профиля (см. соответствующий пункт в `FRONTEND_PLAN.md`, ему на бэке ничего не соответствует).
- **`MediaFileDto.operatorChannel` и `.totalCount`** — захардкожены `null`/`0` (наследие реконструкции контракта с фронта), не похоже, что фронт их реально использует, но стоит перепроверить перед тем как полагаться на них.
