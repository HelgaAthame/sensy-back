...
| GPT-функции (саммари/чек-лист) | **Решено (обновлено): Grok API**, а не self-hosted Ollama | причина смены решения: Render free tier (512MB RAM) физически не тянет даже квантованную 7B-модель — Ollama потребовала бы переезда всего бэка на отдельный сервер (Oracle Cloud Always Free и т.п.), это отдельная миграция. У пользователя есть аккаунт Grok с бесплatным лимитом использования — проще и без переезда с Render | у Grok есть бесплатный лимит запросов, но это платный SaaS API по своей природе (лимит может закончиться при росте нагрузки) — единственное осознанное отступление от принципа "всё self-hosted/бесплатно" в проекте, принятое ради практичности деплоя |
...
- ~~Self-hosted Ollama vs Grok API для GPT-функций~~ → **Grok API** (см. раздел 6). Причина: Render free tier не тянет Ollama по памяти, а переезд ради этого — отдельная большая задача; у пользователя есть Grok с бесплатным лимитом. Единственное отступление от "всё self-hosted".
...
**Фаза 3 — тональность и аналитика. 🚧 В процессе.** Эмоции/негатив по аудио (`onnx-community/wav2vec2-base-Speech_Emotion_Recognition-ONNX`) по чанкам STT, полный расчёт `summaryAnalyserResult` (включая ранее заглушенные поля из Фазы 2 — счётчики/средние по simultaneousSpeech/Silence, разбивка по оператору/клиенту по конвенции канал 0/1). `GET api/v2/dashboard` — ещё не сделан, будет следующим шагом внутри этой фазы.

**Фаза 4 — GPT-функции.** Grok API (см. раздел 6, решение обновлено — не self-hosted Ollama), эндпоинт `POST api/mediafile/{id}/gpt-analysis` (ленивый триггер), применение чек-листа к звонку (`PUT api/mediafile/{id}`). Нужен токен Grok API от пользователя перед стартом.
