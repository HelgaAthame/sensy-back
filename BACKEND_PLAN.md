...
| Эмоции/тональность | Открытая ONNX-модель speech-emotion-recognition с Hugging Face (конвертированная под onnxruntime-node) | под "negative prob" по сегментам | бесплатно, но качество RU-моделей нужно проверять — см. открытый вопрос |
| GPT-функции (саммари/чек-лист) | **Self-hosted LLM через Ollama** (например Qwen2.5 или Llama 3.1 в компактном кванте), дергается по HTTP из NestJS | соответствует полям `gptSummary`/`gptChecklist`, но без платного API | веса открытые — бесплатно, но требует собственных вычислительных мощностей (CPU долго/неточно, для нормального качества нужен GPU) — см. открытый вопрос |
...
| Контейнеризация | **Docker + docker-compose** (api, worker, postgres, redis, minio, ollama) | всё поднимается на своём сервере, без облачных подписок | бесплатно (кроме самого сервера) |
...
**Фаза 4 — GPT-функции.** Self-hosted LLM (Ollama), эндпоинт `POST api/mediafile/{id}/gpt-analysis` (ленивый триггер), применение чек-листа к звонку (`PUT api/mediafile/{id}`).
