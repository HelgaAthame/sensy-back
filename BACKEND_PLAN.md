...
**Фаза 4 — GPT-функции. ✅ Выполнено локально, требуется ручной шаг для прода.** Groq API (`llama-3.3-70b-versatile`, OpenAI-совместимый chat completions), см. раздел 6. `POST api/mediafile/{id}/gpt-analysis` (202, фоновый джоб в отдельной очереди BullMQ) генерирует `gptSummary` и оценки по всем активным чек-листам проекта звонка (по одной записи на пару звонок+чек-лист в новой таблице `MediaFileChecklist`). `PUT api/mediafile/{id}?checklistId=X` — оператор правит оценки поверх GPT, агрегаты пересчитываются. Проверено полностью локально (саммари + оценка критериев с комментариями + ручная правка).
- **Требуется вручную**: добавить `GROQ_API_KEY` в Render Dashboard → Environment (сама я это сделать не могу) — без него `POST .../gpt-analysis` в проде будет падать с понятной ошибкой `500 GROQ_API_KEY не задан`, остальной API не пострадает (см. защитный паттерн из `GroqClient.getApiKey()`, аналогично `StorageService`).
